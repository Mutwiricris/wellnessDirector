...
class ProfitabilityWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            ...
            Stat::make('Revenue Efficiency', number_format($overallMargin['revenue_efficiency'], 1) . '%')
                ->description('Revenue vs operational capacity')
                ->descriptionIcon('heroicon-m-arrow-trending-up')
                ->color($overallMargin['revenue_efficiency'] >= 80 ? 'success' : 'warning'),
        ];
    }
    ...
    private function calculateServiceProfitability(?int $branchId, $startDate, $endDate): array
    {
        ...
        // Service-related expenses (estimate 20% of service revenue for supplies, utilities)
        $serviceExpenses = $serviceRevenue * 0.20;
    ...
    {
        // Total staff hours (estimated based on bookings)
        ...
        $estimatedHours = $totalBookings * 1.5; // Average 1.5 hours per booking
    ...
    private function calculateOverallMargin(?int $branchId, $startDate, $endDate): array
    {
        ...
        $potentialRevenue = $totalRevenue * 1.25; // Assume 25% capacity increase potential
        $revenueEfficiency = $potentialRevenue > 0 ? ($totalRevenue / $potentialRevenue) * 100 : 0;

        return [
            'margin' => (float) $overallMargin,
            'total_transactions' => (int) $totalTransactions,
            'cost_per_transaction' => (float) $costPerTransaction,
            'revenue_efficiency' => (float) $revenueEfficiency,
        ];
    }
